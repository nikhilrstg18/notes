...
Note: The logical OR operator `|` also computes the logical OR of its operands, but always evaluates both operands.
...
Note: The conditional logical operators `&&` and `||` don't support `bool?` operands.
